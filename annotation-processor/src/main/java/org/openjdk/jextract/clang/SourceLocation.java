...
public class SourceLocation extends ClangDisposable.Owned {
    ...
    private Location getLocation(LocationFactory fn) {
        try (var arena = Arena.ofConfined()) {
             MemorySegment file = arena.allocate(Index_h.C_POINTER);
             MemorySegment line = arena.allocate(Index_h.C_INT);
             MemorySegment col = arena.allocate(Index_h.C_INT);
             MemorySegment offset = arena.allocate(Index_h.C_INT);

            fn.get(loc, file, line, col, offset);
            MemorySegment fname = file.get(Index_h.C_POINTER, 0);
            String str = fname.equals(MemorySegment.NULL) ?  null : getFileName(fname);

            return new Location(str, line.get(Index_h.C_INT, 0),
                col.get(Index_h.C_INT, 0), offset.get(Index_h.C_INT, 0));
        }
    }

    private static String getFileName(MemorySegment fname) {
        var filename = Index_h.clang_getFileName(LibClang.STRING_ALLOCATOR, fname);
