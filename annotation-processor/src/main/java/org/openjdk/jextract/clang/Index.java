...
public class Index extends ClangDisposable {
    ...
    public TranslationUnit parseTU(String file, Consumer<Diagnostic> dh, int options, String... args)
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            MemorySegment cargs = args.length == 0 ? null : arena.allocate(Index_h.C_POINTER, args.length);
            for (int i = 0 ; i < args.length ; i++) {
                cargs.set(Index_h.C_POINTER, i * Index_h.C_POINTER.byteSize(), arena.allocateFrom(args[i]));
            }
            MemorySegment outAddress = arena.allocate(Index_h.C_POINTER);
            ...
            MemorySegment tu = outAddress.get(Index_h.C_POINTER, 0);
