...
public class TranslationUnit extends ClangDisposable {
    ...
    public void reparse(Index.UnsavedFile... inMemoryFiles) {
        try (Arena arena = Arena.ofConfined()) {
            ...
            for (int i = 0; i < inMemoryFiles.length; i++) {
                ...
                start.set(Index_h.C_POINTER, FILENAME_OFFSET, arena.allocateFrom(inMemoryFiles[i].file));
                start.set(Index_h.C_POINTER, CONTENTS_OFFSET, arena.allocateFrom(inMemoryFiles[i].contents));
                start.set(Index_h.C_INT, LENGTH_OFFSET, inMemoryFiles[i].contents.length());
            }
            ErrorCode code;
            int tries = 0;
            do {
    ...
    public Tokens tokenize(SourceRange range) {
        try (Arena arena = Arena.ofConfined()) {
            MemorySegment p = arena.allocate(Index_h.C_POINTER);
            MemorySegment pCnt = arena.allocate(Index_h.C_INT);
            Index_h.clang_tokenize(ptr, range.segment, p, pCnt);
            Tokens rv = new Tokens(p.get(Index_h.C_POINTER, 0), pCnt.get(Index_h.C_INT, 0));
            return rv;
        }
    }
    ...
    public class Tokens extends ClangDisposable {
        ...
        public class Token extends ClangDisposable.Owned {
            ...
            public String spelling() {
                var spelling = Index_h.clang_getTokenSpelling(LibClang.STRING_ALLOCATOR, TranslationUnit.this.ptr, segment);
