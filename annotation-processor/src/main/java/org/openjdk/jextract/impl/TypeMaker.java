...
class TypeMaker {
    ...
    static Type makeType(org.openjdk.jextract.clang.Type t, TreeMaker treeMaker) {
        switch(t.kind()) {
            ...
            case Char_S:
            case Char_U:
                return Type.primitive(Type.Primitive.Kind.Char);
            case Short:
                return Type.primitive(Type.Primitive.Kind.Short);
            case Int:
                return Type.primitive(Type.Primitive.Kind.Int);
            case Long:
                return Type.primitive(Type.Primitive.Kind.Long);
            case LongLong:
                return Type.primitive(Type.Primitive.Kind.LongLong);
            case SChar: {
                Type chType = Type.primitive(Type.Primitive.Kind.Char);
                return Type.qualified(Type.Delegated.Kind.SIGNED, chType);
            }
            case UShort: {
                Type chType = Type.primitive(Type.Primitive.Kind.Short);
                return Type.qualified(Type.Delegated.Kind.UNSIGNED, chType);
            }
            case UInt: {
                Type chType = Type.primitive(Type.Primitive.Kind.Int);
                return Type.qualified(Type.Delegated.Kind.UNSIGNED, chType);
            }
            case ULong: {
                Type chType = Type.primitive(Type.Primitive.Kind.Long);
                return Type.qualified(Type.Delegated.Kind.UNSIGNED, chType);
            }
            case ULongLong: {
                Type chType = Type.primitive(Type.Primitive.Kind.LongLong);
                return Type.qualified(Type.Delegated.Kind.UNSIGNED, chType);
            }
            case UChar: {
                Type chType = Type.primitive(Type.Primitive.Kind.Char);
            ...
            case Bool:
                return Type.primitive(Type.Primitive.Kind.Bool);
            case Double:
                return Type.primitive(Type.Primitive.Kind.Double);
            case Float:
                return Type.primitive(Type.Primitive.Kind.Float);
            case Unexposed:
            case Elaborated:
                    ...
                    return Type.pointer(() -> {
                        ...
                        if (decl.isEmpty()) {
                            ...
                            return switch (decl.get()) {
                                case Declaration.Scoped scoped -> Type.declared(scoped);
                                case Declaration.Typedef typedef -> Type.typedef(typedef.name(), typedef.type());
            ...
            case Complex: {
                ...
                return Type.qualified(Type.Delegated.Kind.COMPLEX, __type);
            }
            case Vector: {
                Type __type = makeType(t.getElementType(), treeMaker);
            ...
            case WChar: //unsupported
                return Type.primitive(Type.Primitive.Kind.WChar);
            case Char16: //unsupported
                return Type.primitive(Type.Primitive.Kind.Char16);
            case Half: //unsupported
                return Type.primitive(Type.Primitive.Kind.HalfFloat);
            case Int128: //unsupported
                return Type.primitive(Type.Primitive.Kind.Int128);
            case LongDouble: //unsupported
                return Type.primitive(Type.Primitive.Kind.LongDouble);
            case UInt128: { //unsupported
                Type iType = Type.primitive(Type.Primitive.Kind.Int128);
                return Type.qualified(Type.Delegated.Kind.UNSIGNED, iType);
            }
            case Atomic: {
                Type aType = makeType(t.getValueType(), treeMaker);
                return Type.qualified(Type.Delegated.Kind.ATOMIC, aType);
            }
            default:
                return Type.error(t.spelling());
        }
    ...
    private static final Type.Visitor<Type, Void> lowerFunctionType = new Type.Visitor<>() {
        ...
        public Type visitDelegated(Type.Delegated t, Void aVoid) {
            if (t.kind() == Type.Delegated.Kind.TYPEDEF && t.type() instanceof Type.Array array) {
