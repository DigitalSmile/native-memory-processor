...
class TreeMaker {
    ...
    Declaration addAttributes(Declaration d, Cursor c) {
        ...
        if (!attributes.isEmpty()) {
            d.addAttribute(new Declaration.ClangAttributes(Collections.unmodifiableMap(attributes)));
        }
        return d;
    }
    ...
    public Declaration.Constant createMacro(Position pos, String name, Type type, Object value) {
        ...
        if (value instanceof String) {
            ...
            valueString = "\"" + valueString + "\"";//STR."\"\{valueString}\"";
        } else if (Utils.isPointer(type)) {
            // add pointer cast to make it look different from a numeric constant
            valueString = "(void*) " + valueString;//STR."(void*) \{valueString}";
        }
        DeclarationString.with(macro, "#define " + name + " " + valueString/*STR."#define \{name} \{valueString}"*/);
        return macro;
    }
    ...
    final Type.Declared recordDeclaration(Cursor parent, Cursor recordCursor) {
        ...
        List<Declaration.Variable> pendingBitFields = new ArrayList<>();
        ...
        recordCursor.forEach(fc -> {
            if (Utils.isFlattenable(fc)) {
                if (fc.isBitField()) {
                    ...
                    Declaration.Variable bitfieldDecl = Declaration.bitfield(CursorPosition.of(fc), fc.spelling(), fc.getBitFieldWidth(), fieldType);
                    ...
                    if (!pendingBitFields.isEmpty()) {
                        pendingFields.add(Declaration.bitfields(pendingBitfieldsPos.get(), pendingBitFields.toArray(Declaration.Variable[]::new)));
        ...
        Declaration.Scoped structOrUnionDecl = recordCursor.kind() == CursorKind.StructDecl ?
    ...
    private static boolean isRedundantTypedef(Declaration d) {
        return d instanceof Declaration.Typedef typedef &&
    ...
    private <D extends Declaration> D withNestedTypes(D d, Cursor c, boolean ignoreNestedParams) {
        ...
        List<Declaration.Scoped> nestedDecls = nestedDefinitions.stream()
                .filter(m -> m instanceof Declaration.Scoped)
                .map(Declaration.Scoped.class::cast)
                .toList();
    ...
    private <D extends Declaration> D withDeclarationString(D decl, Cursor cursor) {
        String declString = switch (decl) {
            ...
            case Declaration.Typedef _ -> declarationString(cursor, true); // always expand typedefs
            default -> {
    ...
    private String enumConstantString(String enumName, Declaration.Constant enumConstant) {
        ...
        return "enum " + enumName + "." + enumConstant.name() + " = " + enumConstant.value();//STR."enum \{enumName}.\{enumConstant.name()} = \{enumConstant.value()}";
    }
}
